...
int main(int argc, char** argv){
  ...
  cout << endl << endl << "Fuck you. Boost factorial! "<< endl;
  pro = boost::math::factorial<long double>(num);
 
  ss << setprecision(600) << pro;
  dummy = ss.str();
 
  cout << "Apparently this thing is " << dummy.size() << " digits long." << endl;
  char* digits3 = new char[dummy.size()];
  strcpy(digits3,dummy.c_str());
  sum=0;
  cout << endl;
  for(int i=0;i<dummy.size();i++){
      sum += ((int) digits3[i] - 48);
      cout << digits3[i];
    }
  cout << endl;
  cout << "The sum of digits= " << sum << endl;
